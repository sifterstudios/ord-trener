...
export const GET = async () => {
    ...
    if (!randomWord) {
        return new Response(JSON.stringify({error: 'No words in database'}), {status: 404});
    }

    const similarWords : Word[] = db.prepare(`
    ...
    if (similarWords.length < 3) {
        return new Response(JSON.stringify({error: 'Not enough similar words found'}), {status: 404});
    }
